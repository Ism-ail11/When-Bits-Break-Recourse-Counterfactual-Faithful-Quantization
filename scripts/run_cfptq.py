...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run training-free CF-PTQ experiments")
    parser.add_argument("--config", default="configs/base.yaml")
    parser.add_argument("--datasets", nargs="+", default=["adult", "german", "compas"])
    parser.add_argument("--methods", nargs="+", default=["ptq8", "ptq4", "mixedptq", "cfptq", "cfptq_sensitivity", "cfq"])
    parser.add_argument("--output", default="results/cfptq")
    ...
    for dataset in args.datasets:
        for method in args.methods:
            ...
            rows.append({"dataset": dataset, "method": method, **result["metrics"], "bits": result["quantized_average_bits"]})
